...
const embeddings = new OpenAIEmbeddings({
  ...
  // 显式指定 float，让接口直接返回浮点数组，向量才正常（否则相似度全 NaN）
...
for (const question of questions) {
  ...
  retrievedDocs.forEach((doc, i) => {
    ...
    // MemoryVectorStore.similaritySearchWithScore 返回的就是余弦相似度（越大越像，范围 -1~1）
    // 不需要用 1 - score 转换（那是把距离转相似度才用的）
    const similarity = scoredResult ? scoredResult[1].toFixed(4) : "N/A";
